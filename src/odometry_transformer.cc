...
void OdometryTransformer::receiveOdometry(
  ...
  if (lookup_tf_) {
    // look up the tf continuously, but settle for the most recent rather
    // than the input odom header stamp
    // TODO(lucasw) make it optionally use the header stamp and have a configurable timeout
    try {
      T_ST_ = tf2::transformToEigen(tf_buffer_.lookupTransform(
          source_frame_, target_frame_, ros::Time(0), ros::Duration(0.0)));
      if (last_lookup_failed_) {
        ROS_WARN_THROTTLE(4.0, "now looking up transform");
      }
      last_lookup_failed_ = false;
    } catch (tf2::TransformException &ex) {
      if (!last_lookup_failed_) {
        ROS_WARN_THROTTLE(4.0, "Cannot lookup transform: %s", ex.what());
      }
      last_lookup_failed_ = true;
    }
  }
